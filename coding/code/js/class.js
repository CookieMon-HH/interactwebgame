...
class Tank {
    ...
    keyMotion(){                
        ...
        if(this.checkdirection() != 'notmoving'){
            ...
            this.movex = this.movex + this.speed * Math.cos(degtorad(this.rotateangle));
            this.movey = this.movey - this.speed * Math.sin(degtorad(this.rotateangle));
        }else {
...
class Bullet {
    constructor(){
		...
		this.el.className = 'tank_bullet_box';
    ...
    init(){
        this.x = tank.position().left + tank.size().width * 0.5 ;
        this.y = -tank.position().bottom - tank.size().width * 0.42 ;
        ...
        //각도 조절
        this.rotateangle = rotateanglelist[directionstatus] ;
    ...
    moveBullet(){
        ...
        this.el.style.transform = `translate3d(${this.distancex}px, ${this.distancey}px, 0)`;
    }
}