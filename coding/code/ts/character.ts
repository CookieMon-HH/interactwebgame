...
class Character {
  ...
  keyMotion = () => {
    ...
    if (key.keyDown['slide']) {
      if(this._slideMotion.isKeyDown) return;

      this._element.classList.add('slide');
      if (this._direction === CharacterDirection.RIGHT) {
