const sameFunction = (imgUrl: string, script: string) => {
  return (
    `
...
const makeBasicScript = (script: string) => {
  return (
    `
    <div class="talk_box">
      ${script}
...
const NpcQuestSet: Record<keyof typeof NpcQuestType, (imgUrl: string, script: string) => string> = {
...
interface IQuestInfo extends IQuestMessage{
  url: string;
  possibleLevel : number;
}

const NpcQuestInfoSet: Record<keyof typeof NpcType, IQuestInfo> = {
  NPC_YELLOW: {
...
class Npc {
  ...
  private _talkOn: boolean = false;
  private _talkFirst: boolean = true;
  private _questEnd: boolean = false;
  private _modalElement: HTMLElement;
  private _NpcType: NpcType;

  constructor(type: NpcType, startPositionX: number) {
  ...
  init = (positionX : number) => {
    this._element.innerHTML = NpcQuestSet[NpcQuestType.BASIC](NpcQuestInfoSet[this._NpcType].url, NpcQuestInfoSet[this._NpcType].basic);
    ...
    if (this._isCrash) {
      if (!this._talkOn) {
        ...
        if (this._talkFirst) {
          ...
          this.makeQuestScript(NpcQuestType.START, NpcQuestInfoSet[this._NpcType].url, NpcQuestInfoSet[this._NpcType].start);
        } else {
          if (this._questEnd) {
            this.makeQuestScript(NpcQuestType.CLEAR, NpcQuestInfoSet[this._NpcType].url, NpcQuestInfoSet[this._NpcType].clear);
          } else {
            if (character.currentLevel < NpcQuestInfoSet[this._NpcType].possibleLevel)
              this.makeQuestScript(NpcQuestType.IMPOSSIBLE, NpcQuestInfoSet[this._NpcType].url, NpcQuestInfoSet[this._NpcType].possible);
            else {
              this.makeQuestScript(NpcQuestType.POSSIBLE, NpcQuestInfoSet[this._NpcType].url, NpcQuestInfoSet[this._NpcType].impossible);
              this._questEnd = true;
            }
          }
        }
  ...
  makeQuestScript = (type: NpcQuestType, url: string, questScript: string) => {
    const script = NpcQuestSet[type](url, questScript)
    this.setQuestScript(script);
  }

  setQuestScript = (script: string) => {
